...
public class DestinationController {
    ...
    public ModelAndView getDestinationDetails(@PathVariable("countryName") String countryName) {
        DestinationViewDTO destinationViewDTO = this.destinationService.getDestinationByCountryName(countryName);

        ModelAndView modelAndView = new ModelAndView("destination-details");

        modelAndView.addObject("destination", destinationViewDTO);
